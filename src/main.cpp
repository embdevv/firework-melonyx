    ...
    int main() {
        ...
        if (!engine.sphereMesh.load("3D/sphere.obj"))
        { 
		    std::cerr << "Failed to load sphere mesh\n";
            return -1;
        }

	    engine.sphereMesh.upload();
        ...
        while (!glfwWindowShouldClose(win)) {
            ...
            for (const auto& particle : engine.getParticles()) {
                ...
                glDrawElements(GL_TRIANGLES,(GLsizei)engine.sphereMesh.indices.size(), GL_UNSIGNED_INT, 0);
            }
            glBindVertexArray(0);
